...
export const CommunityProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const fetchJoinedCommunities = async () => {
    try {
      ...
      const response = await fetch(`${config.apiUrl}/communities/joined`, {
  ...
  const joinCommunity = async (communityId: string): Promise<boolean> => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`${config.apiUrl}/communities/${communityId}/join`, {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${token}` }
      });
  ...
  const leaveCommunity = async (communityId: string): Promise<boolean> => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`${config.apiUrl}/communities/${communityId}/leave`, {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${token}` }
      });
